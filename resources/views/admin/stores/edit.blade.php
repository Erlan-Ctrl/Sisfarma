...
@section('title', 'Editar loja | Admin')
@section('heading', 'Editar loja')
@section('subtitle', $store->name)

@section('content')
    <form class="grid gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm" action="{{ route('admin.stores.update', $store) }}" method="post">
        @csrf
        @method('PUT')
        ...
        <div class="grid gap-4 md:grid-cols-2">
            <div class="grid gap-2">
                ...
                <input class="rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-slate-900 focus:ring-2 focus:ring-slate-200" type="text" name="opening_hours" value="{{ old('opening_hours', $store->opening_hours) }}">
            </div>
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            <a class="text-sm font-semibold text-slate-700 hover:text-slate-900" href="{{ route('admin.stores.index') }}">Voltar</a>
            <a class="ml-auto rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold shadow-sm hover:bg-slate-50" href="{{ route('admin.stores.show', $store) }}">
                Ver
            </a>
        </div>
    </form>
