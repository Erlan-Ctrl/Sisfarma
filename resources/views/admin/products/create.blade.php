...
@section('subtitle', 'Cadastrar produto')

@section('content')
    <form class="grid gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm" action="{{ route('admin.products.store') }}" method="post">
        @csrf
        ...
        <div class="grid gap-4 md:grid-cols-3">
            ...
            <div class="grid gap-2">
                ...
                <div class="flex items-center gap-2">
                    <input id="product-ean" class="flex-1 rounded-xl border border-slate-200 px-4 py-2.5 font-mono text-xs outline-none focus:border-slate-900 focus:ring-2 focus:ring-slate-200" type="text" name="ean" value="{{ old('ean', request('ean', $product->ean)) }}">
        ...
        <div class="grid gap-2">
            ...
            <select class="rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-slate-900 focus:ring-2 focus:ring-slate-200" name="supplier_id">
                ...
                @foreach ($suppliers as $supplier)
                    <option value="{{ $supplier->getKey() }}" @selected((int) old('supplier_id') === (int) $supplier->getKey())>
                        {{ $supplier->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="grid gap-2">
            <label class="text-sm font-medium text-slate-700">Descrição curta (opcional)</label>
        ...
        <div class="grid gap-2">
            ...
            <input class="rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-slate-900 focus:ring-2 focus:ring-slate-200" type="text" name="image_url" value="{{ old('image_url', $product->image_url) }}" placeholder="https://...">
        </div>
        ...
        <div class="grid gap-4 md:grid-cols-2">
            ...
            <label class="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm">
                <input type="checkbox" name="requires_prescription" value="1" @checked(old('requires_prescription', false))>
        ...
        <div class="grid gap-2">
            ...
            <select class="min-h-24 rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-slate-900 focus:ring-2 focus:ring-slate-200" name="category_ids[]" multiple>
                @foreach ($categories as $category)
                    <option value="{{ $category->id }}" @selected(in_array($category->id, old('category_ids', []), true))>
                        {{ $category->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            <a class="text-sm font-semibold text-slate-700 hover:text-slate-900" href="{{ route('admin.products.index') }}">Cancelar</a>
        </div>
    </form>
    ...
    <script>
        (function () {
            ...
            // Auto-prefill when coming from Scanner with ?ean=
            if ((eanInput.value || '').trim() !== '' && (nameInput.value || '').trim() === '') {
                lookup();
            }
        })();
    </script>
